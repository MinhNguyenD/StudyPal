            ...
            <form class="w-full max-w-lg mt-10" @submit.prevent="updatePassword">
                <div class="flex flex-wrap -mx-3 mb-5">
                    <div class="w-full px-3">
                        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="currentPassword">
                            Current Password
                        </label>
                        <input class="appearance-none block w-full h-10 bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="currentPassword" type="password" v-model="currentPassword" required />
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-5">
                    <div class="w-full px-3">
                        ...
                        <input class="appearance-none block w-full h-10 bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="newPassword" type="password" v-model="newPassword" required />
                        <p class="text-red-500 text-xs italic" v-if="newPasswordError.length > 0">{{ newPasswordError }}</p>
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-5">
                    <div class="w-full px-3">
    ...
    <div v-if="updateSuccessfully" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
        <div class="bg-white rounded-lg p-6 w-96">
            ...
            <div class="mt-6 text-right">
                <button @click="goToLogin" class="bg-primary text-white py-2 px-4 rounded hover:bg-blue-700">OK</button>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        async updatePassword() {
            ...
            try {
                ...
                const updatePassword = await axios.put(`update/user/password/${currentUser.username}`, {
        ...
        validateForm() {
            ...
            if (!passwordRegex.test(this.newPassword)) {
                this.newPasswordError = "Password must be at least 8 characters long and include at least one digit, 1 lowercase letter, 1 uppercase letter, and 1 special character";
                formValidated = false;
            }
            else {
