...
<template>
    <div class="py-10 space-y-2" v-if="slotData.data.isSelected">
        <p class="text-3xl font-semibold">Study Group</p>
        <br>
        <p class="text-xl">Members: {{ slotData.data.users.length }}</p>
        <button :disabled="isInGroup" @click="joinGroup" :class="{'opacity-50 cursor-not-allowed': isInGroup}" class="bg-primary text-white py-2 px-4 rounded p-2">Join study group</button>
        <button v-if="isInGroup" @click="leaveGroup" class="mx-2 bg-red-500 text-white py-2 px-4 rounded p-2">Leave study group</button>
    </div>
    <div v-else class="py-10 w-60 space-y-2">
        <p class="text-xl font-semibold">There is no group in current time slot right now</p>
        <button  :class="{'opacity-50 cursor-not-allowed': isInGroup}" @click="createGroup" class="bg-green-500 p-2">Create new group</button>
    </div>
</template>
<style>
